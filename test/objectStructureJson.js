...
describe('get specific map item to check object structure (JSON)', function () {
  ...
  const checkProperties = [ 'ids', 'bb', 'center', 'tags', 'skel', 'geom', 'meta geom' ]
  ...
  checkProperties.forEach(outOptions => {
    it('check with out options "' + outOptions + '"', function (done) {
      ...
      async.each(items, (id, done) => {
        const file = 'test/reference/' + id + '-' + outOptions.replace(/ /g, '_') + '.json'
        ...
        async.parallel({
          ...
          if (err) { return done(err) }
          assert.deepEqual(actual, expected, 'Output of item ' + id + ' is wrong')
          done()
        })
      }, (err) => done(err))
    })
